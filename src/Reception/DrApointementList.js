...
export default function DrApointmentList(){
  ...
  const userData = useSelector(state=>state.userDoctor.value)
    ...
    var done = async (id)=>{
      const rspn = await WebMethodes.putApi(WebAPI.apointmentDone+id,null,userData.token)
      if(rspn.data.status){
    ...
    return <div>
      ...
      <h5 className="mt-3 text-center">Total Patients : {apointmentData.reduce((count,ob)=>ob.activeStatus==currentStatus && checkDate(ob.appointmentdate) ?count+=1:count,0)}</h5>
      </div>
      {apointmentData.filter(ob=>ob.activeStatus==currentStatus && checkDate(ob.appointmentdate)).map(ob=><section className="book_section layout_padding">
        ...
        <div className="col">
          <div>
            ...
            <div className="form-group col-lg-4"></div>
              ...
              {currentStatus?
                <button onClick={()=>done(ob.id)} className="btn btn-info">DONE</button>
                : 
